    ...
    while True:
        ...
        if data == 'Join Queue':
            ...
            if len(queue) == 0:
                ...
                d1 = random.randint(1, len(host_ser)-1)
                d2 = random.randint(1, len(host_ser)-1)

                s1 = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s1.connect((host_ser[d1], 8000))
                ...
                if d1_num > d2_num:
                    d = d2
                    print('Choose server', d2)
                else:
                    d = d1
                    print('Choose server', d1)
